...
cmd:option('-batchsize', 1, 'number of examples in minibatch')
...
cmd:option('-max_dilation', 128 , 'max dilation in one block , e.g. 1, 2, 4, 8, ...')
cmd:option('-dilated_nummaps', '4,4,4,4,4,4,4,4', 'num maps for dilated convs in one expblock; need n, where 2^(n-1) = max_dilation')
cmd:option('-onedim_nummaps', 4, 'num maps for 1x1 convolutions in res block')
cmd:option('-output_nummaps', 8, 'number of feature maps for 1x1 convs in output')
-- cmd:option('-output_nummaps', '32,16', 'number of feature maps for 1x1 convs in output')
-- Optimization
cmd:option('-method','adam', 'which optimization method to use')
...
cmd:option('-save_model_every_epoch', 50, 'how often to save model')
